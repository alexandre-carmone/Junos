...
.files-header {
    display:flex; align-items:center; gap:12px; padding:10px 20px 10px 80px;
    border-bottom:1px solid #222; background:rgba(6,6,15,0.85);
    font-size:13px; min-height:44px;
}
.files-title { color:#88aaff; font-weight:600; letter-spacing:0.06em; }
.files-grow { flex:1; }
.files-status { color:#88aaff; font-size:11px; }
.files-breadcrumb { color:#c0c0d0; }
.files-crumb {
    background:transparent; border:none; color:#88aaff; cursor:pointer;
    font:inherit; padding:2px 4px; border-radius:3px;
}
.files-crumb:hover { background:rgba(136,170,255,0.15); }
.files-crumb-active { color:#cfe0ff; }
.files-crumb-sep { color:#555; }
...
@media (max-width: 900px) {
    .files-body { grid-template-columns: 160px 1fr 280px; }
}
@media (max-width: 700px) {
    .files-body { grid-template-columns: 1fr; grid-template-rows: auto auto auto;
                  overflow-y:auto; }
}
...
.files-row {
    ...
    background:transparent; color:#c0c0d0; border:1px solid transparent;
    border-radius:4px; cursor:pointer; font:inherit;
}
.files-row:hover { background:rgba(136,170,255,0.08); border-color:#2a2a35; }
.files-folder-icon { font-size:14px; }
.files-row-name { white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.files-parent { color:#88aaff; }
...
.files-thumb {
    background:#06060c; border:1px solid #2a2a35; border-radius:4px;
    overflow:hidden; cursor:pointer; padding:0; color:inherit; font:inherit;
    display:flex; flex-direction:column;
}
.files-thumb:hover { border-color:#88aaff; }
.files-thumb-active { border-color:#88aaff; box-shadow:0 0 0 2px rgba(136,170,255,0.25); }
.files-thumb-img {
    width:100%; aspect-ratio:1/1; object-fit:cover; background:#000;
    display:block;
}
.files-thumb-name {
    font-size:10px; padding:4px 6px; color:#a0a0b0;
    white-space:nowrap; overflow:hidden; text-overflow:ellipsis;
}
.files-empty { color:#666; padding:20px; text-align:center; }

.files-detail {
    border-left:1px solid #222; overflow-y:auto; padding:12px;
    background:rgba(6,6,15,0.6);
}
.files-no-sel { color:#666; padding:20px; text-align:center; font-size:12px; }
.files-preview {
    width:100%; max-height:260px; object-fit:contain; background:#000;
    border:1px solid #2a2a35; border-radius:4px; margin-bottom:12px;
}
.files-section {
    color:#88aaff; font-size:11px; text-transform:uppercase;
    letter-spacing:0.06em; margin:14px 0 6px; font-weight:600;
}
.files-kv-list { display:grid; gap:3px; font-size:12px; }
.files-kv { display:flex; justify-content:space-between; gap:8px; }
.files-kv-k { color:#88aaff; }
.files-kv-v { color:#d0d0e0; text-align:right; word-break:break-word; }

.files-collapsible {
    margin-top:10px; border:1px solid #2a2a35; border-radius:4px;
    background:rgba(6,6,15,0.4); padding:6px 8px;
}
.files-collapsible summary {
    cursor:pointer; color:#88aaff; font-size:11px;
...
.files-fits-row {
    display:grid; grid-template-columns:80px 1fr 1.2fr; gap:6px;
    padding:1px 0; border-bottom:1px dotted #1a1a25;
}
.files-fits-key { color:#88aaff; }
.files-fits-val { color:#d0d0e0; word-break:break-all; }
.files-fits-cmt { color:#666; font-style:italic; word-break:break-all; }

.files-livestack { margin-top:18px; padding-top:12px; border-top:1px solid #222; }
.files-ls-buttons { display:flex; gap:6px; flex-wrap:wrap; margin-bottom:8px; }
.files-btn {
    background:rgba(20,24,40,0.9); color:#cfe0ff; border:1px solid #2a2a35;
    padding:5px 10px; border-radius:4px; font:inherit; font-size:12px;
    cursor:pointer;
}
.files-btn:hover { border-color:#88aaff; }
.files-btn-primary { background:rgba(40,60,110,0.95); border-color:#88aaff; }
.files-ls-status { font-size:12px; min-height:22px; }
.files-ls-idle { color:#666; }
.files-ls-stats { display:flex; gap:14px; flex-wrap:wrap; align-items:center; }
.files-ls-badge {
    background:rgba(40,60,110,0.95); border:1px solid #88aaff;
    padding:2px 8px; border-radius:10px; font-size:11px; color:#cfe0ff;
}

.files-ls-form { display:flex; flex-direction:column; gap:8px; padding:8px 0; }
.files-field { display:flex; flex-direction:column; gap:3px; font-size:11px; color:#88aaff; }
.files-field-row { flex-direction:row; align-items:center; gap:8px; color:#c0c0d0; font-size:12px; }
.files-field input[type="text"], .files-field input[type="number"] {
    background:#06060c; color:#c0c0d0; border:1px solid #2a2a35;
    padding:4px 6px; border-radius:3px; font:inherit; font-size:12px;
}
.files-field input:focus { outline:none; border-color:#88aaff; }
.files-error { color:#ff8888; padding:8px; font-size:12px; }
"#;
