...
pub trait SkyLayer {
    ...
    ///
    /// Takes `&mut Frame` so layers can append to `hit_items`. Layers that
    /// don't mutate frame state should still match this signature (just
    /// don't touch the mut fields).
