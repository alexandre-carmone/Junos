//! Deep-sky-object overlay: outline symbols, labels, and DSO hit items.
//!
//! In `Gpu` mode the GPU `DsoLayer` (instances built via
//! `dso_render::build`) draws the outline symbols and the `TextLayer`
//! draws labels — but DSO hit items still flow through `render_dso`'s
//! Canvas2D path. So this layer always runs `draw_canvas2d`, regardless
//! of mode; `render_dso` itself gates its symbol/label work on the mode.
//! Both paths take their symbol geometry from `dso_shape`.
...
impl SkyLayer for DsoLayer {
    ...
    fn draw_canvas2d(&self, f: &mut Frame, ctx: &CanvasRenderingContext2d) {
        ...
        // `view` doesn't conflict with the mutable borrow of `hit_items`.
        let view = *f.view;
        ...
        render_dso(ctx, f, &dso_cat_owned, dso_index, &proj, scale);
    }
}
